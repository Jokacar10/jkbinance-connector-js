...
async function deleteMarginCallLevel() {
    try {
        const response = await client.restAPI.deleteMarginCallLevel();

        const rateLimits = response.rateLimits!;
        console.log('deleteMarginCallLevel() rate limits:', rateLimits);

        const data = await response.data();
        console.log('deleteMarginCallLevel() response:', data);
    } catch (error) {
        console.error('deleteMarginCallLevel() error:', error);
    }
}
